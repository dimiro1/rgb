...
pub fn push_de(state: &mut State) {
...
pub fn call_nc(state: &mut State) {
    if !state.flag_c() {
        call(state);
    } else {
        // Skip the 2-byte address
...
pub fn ldh_c_a(state: &mut State) {
